...
using CQRSSqlServer.Api.Infrastructure.Helpers;
using Dapper;
using System;
...
namespace CQRSSqlServer.Api.Application.Queries
{
    public class DocenteQueries : IDocenteQueries
    {
        ...
        public async Task<PaginatedItemsResponse<DocenteResponseDto>> ListarDocentes(PaginatedItemsRequest<DocenteRequestDto> request)
        {
            ...
            using (var connection = new SqlConnection(_connectionString))
            {
                ...
                return new PaginatedItemsResponse<DocenteResponseDto>(request.Skip, request.PageSize, count, rpta);
            }


        }

        public async Task<PaginatedItemsResponse<DocenteResponseDto>> ListarDocentesSP(PaginatedItemsRequest<DocenteRequestDto> request)
        {
            long count = 0;
            var rpta = new List<DocenteResponseDto>();
            ...
            using (var connection = new SqlConnection(_connectionString))
            {
                ...
                var result = await connection.QueryAsync<dynamic>("[dbo].[USP_SEL_FILIAL_LISTAR]", param: parameter, commandType: CommandType.StoredProcedure);
